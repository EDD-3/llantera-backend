...
class Usuario(db.Model):
    ...
    empleado_id = db.Column(
        db.Integer, db.ForeignKey("empleado.id", ondelete="SET NULL"), nullable=True
    )
    tipo_usuario_id = db.Column(
        db.Integer, db.ForeignKey("tipo_usuario.id", ondelete="SET NULL"), nullable=True
    )
    reparaciones = db.relationship(
        "Reparacion", backref="usuario", cascade="all, delete", lazy="subquery"
    )  # one-to-many
    empleado = db.relationship(
        "Empleado", back_populates="usuario", lazy="subquery"
    )  # one-to-one
...
class TipoUsuario(db.Model):
    ...
    usuarios = db.relationship(
        "Usuario", backref="tipo_usuario", lazy="subquery"
    )  # many-to-one
...
class Reparacion(db.Model):
    ...
    usuario_id = db.Column(
        db.Integer, db.ForeignKey("usuario.id", ondelete="SET NULL"), nullable=True
    )
    cliente_id = db.Column(
        db.Integer, db.ForeignKey("cliente.id", ondelete="SET NULL"), nullable=True
    )
    garantia_id = db.Column(
        db.Integer, db.ForeignKey("garantia.id", ondelete="SET NULL"), nullable=True
    )
    total = db.Column(db.Numeric(precision=2))
    fecha_realizacion = db.Column(
        db.DateTime, nullable=False, default=datetime.now() + timedelta(hours=-6)
    )
    reparaciones_detalle = db.relationship(
        "ReparacionDetalle",
        backref="reparacion",
        cascade="all, delete",
        lazy="subquery",
...
class Cliente(db.Model):
    ...
    reparaciones = db.relationship(
        "Reparacion", backref="cliente", lazy="subquery"
    )  # one-to-many
...
class Vehiculo(db.Model):
    ...
    fecha_fabricacion = db.Column(YEAR)
    cliente_id = db.Column(
        db.Integer, db.ForeignKey("cliente.id", ondelete="SET NULL"), nullable=True
    )
    descripcion = db.Column(db.String(128))
    ...
    cliente = db.relationship(
        "Cliente", back_populates="vehiculo", lazy="subquery"
    )  # one-to-one
...
class ReparacionDetalle(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    parte_id = db.Column(
        db.Integer, db.ForeignKey("parte.id", ondelete="SET NULL"), nullable=True
    )
    reparacion_id = db.Column(
        db.Integer, db.ForeignKey("reparacion.id", ondelete="SET NULL"), nullable=True
    )
...
class Parte(db.Model):
    ...
    tipo_parte_id = db.Column(
        db.Integer, db.ForeignKey("tipo_parte.id", ondelete="SET NULL"), nullable=True
    )
    descripcion_parte = db.Column(db.String(128))
    precio = db.Column(db.Numeric(precision=2))
    reparaciones_detalle = db.relationship(
        "ReparacionDetalle", backref="parte", lazy="subquery"
    )  # one-to-many
...
class TipoParte(db.Model):
    ...
    partes = db.relationship(
        "Parte", backref="tipo_parte", lazy="subquery"
    )  # one-to-many
...
class Inventario(db.Model):
    ...
    cantidad = db.Column(db.Integer)
    parte = db.relationship(
        "Parte", back_populates="inventario", lazy="subquery"
    )  # one-to-one
